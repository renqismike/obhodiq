...
> [!IMPORTANT]
> Obhodiq — это **не самостоятельный VPN-клиент** и **не VPN-сервис**. Он не предоставляет серверы, не поднимает туннели, не выполняет маршрутизацию трафика сам по себе и не даёт доступ к каким-либо ресурсам без уже установленного Podkop.
>
> Технически Obhodiq делает только одно: **обрабатывает ссылку подписки и подготавливает конфигурационные данные для Podkop**. Фактическое подключение, выбор маршрута, `URLTest`, transport-логика и обработка трафика выполняются уже самим Podkop и его зависимостями.
...
> [!NOTE]
> Проект предназначен для **технической обработки подписок и управления конфигурацией Podkop на OpenWrt**.
>
> Сам Obhodiq:

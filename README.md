...
> [!NOTE]
> Obhodiq сам по себе **не является VPN-клиентом, не содержит собственного tunnel/proxy-core и не выполняет маршрутизацию трафика**. Это парсер и слой интеграции над Podkop: он забирает подписку, разбирает данные и подготавливает их для Podkop. Фактическое подключение, `URLTest`, маршрутизация и transport-логика выполняются уже самим Podkop и его зависимостями.

> [!CAUTION]
> Этот README описывает техническое назначение проекта и **не является юридической консультацией**. Если для тебя важны требования законодательства или правила использования VPN/прокси в твоей юрисдикции, их нужно проверять отдельно.
...
Для части провайдерских обёрток в Obhodiq также используется эмуляция клиента `HApp 1.0`, чтобы некоторые subscription-endpoint вообще начали отдавать ожидаемый ответ.
...
- если у сервера нет пинга, это значит не то, что ссылка обязательно битая, а то, что именно Podkop не вернул по нему нормальный latency в текущей конфигурации
...
- не каждая провайдерская подписка обязана совпадать с теми вариантами, которые уже проверялись в Obhodiq
...
Во время последних проверок отдельно подтверждалось:

- чистая установка `apk` после полного удаления Obhodiq
- чистая установка `ipk` на реальный роутер после полного удаления Obhodiq
- удаление Obhodiq без удаления и поломки самого Podkop

...
set "LOCAL_OUTPUT=obhodiq_%NEW_VERSION%_all.ipk"
...
powershell -NoProfile -Command "$content = Get-Content '%WORK%\control\control' -Raw; $content = [regex]::Replace($content, '^Version: .*$','Version: %NEW_VERSION%','Multiline'); $content = [regex]::Replace($content, '^Source: .*(\r?\n)?','', 'Multiline'); $content = [regex]::Replace($content, '^SourceName: .*(\r?\n)?','', 'Multiline'); $content = [regex]::Replace($content, '^SourceDateEpoch: .*(\r?\n)?','', 'Multiline'); $content = [regex]::Replace($content, '^Maintainer: .*$','Maintainer: renqismike','Multiline'); [System.IO.File]::WriteAllText('%WORK%\control\control', $content, (New-Object System.Text.UTF8Encoding($false)))"
...
del /f /q "%WORK%\control.tar.gz" "%WORK%\data.tar.gz" "%WORK%\%LOCAL_OUTPUT%" "%OUTPUT_IPK%" 2>nul
...
(
  cd /d "%WORK%"
  tar -cf "%LOCAL_OUTPUT%" debian-binary control.tar.gz data.tar.gz
) || exit /b 1

copy /y "%WORK%\%LOCAL_OUTPUT%" "%OUTPUT_IPK%" >nul || exit /b 1
